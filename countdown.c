...
static void show_time(void) {
  ...
  static char time_test[9];
    ...
    APP_LOG(APP_LOG_LEVEL_DEBUG, "show_time %02d", sec);

  text_layer_set_text(tl_zeit, body_text);
 /* text_layer_set_color(tl_zeit, GColorWhite);. */
  if (minutes < 10 )
  {
...
static void window_load(Window *window)
{
 ...
 // GRect bounds = layer_get_frame(window_layer);
  ...
  tl_zeit = text_layer_create(GRect(0,20, 144, 75));  
  ...
  tl_down = text_layer_create(GRect(0,95, 144, 75));  
